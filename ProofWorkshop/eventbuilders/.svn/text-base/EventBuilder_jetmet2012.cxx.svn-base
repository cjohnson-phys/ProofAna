...
Bool_t EventBuilder_jetmet2012::CopyClusters(){
  if(doLCCluster){
    ...
    if(doSMWZ ){
      ...
      beta = BranchKey("cl_lc_eta");
      bphi = BranchKey("cl_lc_phi");
    for(int iCl = 0; iCl < Get<vector<float> >(bpt).size(); iCl++){
...
Bool_t EventBuilder_jetmet2012::CopyTracks(){
  ...
  static const BranchKey btrk_n("trk_n"), btrk_pt("trk_pt"), btrk_eta("trk_eta"), btrk_nPixHits("trk_nPixHits"),
    ...
    //btrk_nTRTOutliers("trk_nTRTOutliers"), 
    btrk_nTRTHits("trk_nTRTHits"),
    //btrk_eta_atCalo("trk_eta_atCalo"), btrk_phi_atCalo("trk_phi_atCalo"),
    btrk_mc_barcode("trk_mc_barcode");
...
Bool_t EventBuilder_jetmet2012::CopyVertices()
{
  ...
  static const MomKey mvtxs("vtxs"), ntrk("ntrk"), mNPV("NPV"), sumPt("sumPt"),
    chisq("chisq"), ndof("ndof");

  static const BranchKey bvxp_nTracks("vxp_trk_n"), bvxp_x("vxp_x"), bvxp_y("vxp_y"), 
    bvxp_z("vxp_z"), bvxp_chisq("vxp_chi2"), bvxp_ndof("vxp_ndof"),// bvxp_sumpt("vxp_sumPt"), 
    bvxp_trk_index("vxp_trk_index");
...
Bool_t EventBuilder_jetmet2012::CopyJet(TString jetType, TString jetClass) 
{
  ...
  // Handle track jets
    ...
    // add constits
    jet->AddVec("constit");
    jet->Set("constscale_pt", Get<vector<float> >("jet_" + jetType + "_pt").at(iJet));
...
Bool_t EventBuilder_jetmet2012::CopyMuons()
{
  ...
  static const BranchKey bmu_muid_n("mu_muid_n"), bmu_muid_pt("mu_muid_pt"), bmu_muid_eta("mu_muid_eta"), bmu_muid_phi("mu_muid_phi"), bmu_muid_E("mu_muid_E"),
    bmu_muid_me_qoverp_exPV("mu_muid_me_qoverp"), bmu_muid_id_qoverp_exPV("mu_muid_id_qoverp_exPV"),
    bmu_muid_me_theta_exPV("mu_muid_me_theta"), bmu_muid_id_theta_exPV("mu_muid_id_theta_exPV"), bmu_muid_id_theta("mu_muid_id_theta"),
...
Bool_t EventBuilder_jetmet2012::CopyStandardJet(int JetFlag){
  ...
  const BranchKey bjet_JetType_pt(lead+bjets+"_pt"), bjet_JetType_eta(lead+bjets+"_eta"), 
    ...
    bjet_JetType_Timing(lead+bjets+"_Timing"), 
    //bjet_JetType_sumPtTrk(lead+bjets+"_sumPtTrk"+fix),
