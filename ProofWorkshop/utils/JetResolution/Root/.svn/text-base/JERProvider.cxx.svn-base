...
JERProvider::JERProvider(std::string CollectionName, std::string MethodName, std::string FileName):
  m_collectionName(CollectionName), m_methodName(MethodName), m_fileName(FileName), m_isInit(false),GeV(1000),m_is7TeV(false)
{
  //Nothing needed here
}
...
double JERProvider::getRelResolutionMC(double pt, double eta) {
  ...
  if (pt*cosh(eta)>10000.0*GeV) 
    fatal(Form("I refuse! You are giving me a jet with E=%.2f TeV. Can't be right. Check your units. OR this might be new physics!",
	       pt*cosh(eta)/GeV/1000));
  if (pt*cosh(eta)>4200.0*GeV) warning(Form("You are using a large jet: E= %.2f TeV.",pt*cosh(eta)/GeV/1000));
  if (pt/GeV < 10 ) pt=10.0*GeV; if (pt/GeV > 2000 ) pt=2000.0*GeV;
  return m_jerFunc.at(getEtaBin(eta))->Eval(pt/GeV);
}
...
double JERProvider::getRelResolutionMC_AFII(double pt, double eta) {
  ...
  if (pt/GeV < 10 ) pt=10.0*GeV; if (pt/GeV > 2000 ) pt=2000.0*GeV;
  return m_jerFuncAFII.at(getEtaBin(eta))->Eval(pt/GeV);
}
...
double JERProvider::getInsituRelResolutionData(double pt, double eta) {
  ...
  if (pt>1000*GeV) pt=1000*GeV; if (fabs(eta)>2.1 && pt>300*GeV) pt=300*GeV;
  return m_jerData[etaBin]->Eval(pt/GeV);
}
...
double JERProvider::getInsituRelResolutionMC(double pt, double eta) {
  ...
  if (etaBin>3) etaBin=3; // due to lacking of stats from data/mc
  if (pt>1000*GeV) pt=1000*GeV; if (fabs(eta)>2.1 && pt>300*GeV) pt=300*GeV;
  return m_jerMC[etaBin]->Eval(pt/GeV);
}
...
double JERProvider::getInsituUncertData(double pt, double eta) {
  ...
  if (etaBin>3) etaBin=3; // due to lacking of stats from data/mc
  if (pt>1000*GeV) pt=1000*GeV; if (fabs(eta)>2.1 && pt>300*GeV) pt=300*GeV;
  return m_errorData[etaBin]->Eval(pt/GeV);
}
...
double JERProvider::getInsituUncertMC(double pt, double eta) {
  ...
  if (etaBin>3) etaBin=3; // due to lacking of stats from data/mc
  if (pt>1000*GeV) pt=1000*GeV; if (fabs(eta)>2.1 && pt>300*GeV) pt=300*GeV;
  return m_errorMC[etaBin]->Eval(pt/GeV);
}
...
double JERProvider::getInsituDiffDataMC(double pt, double eta) {
  ...
  if (pt>1000*GeV) pt=1000*GeV; if (fabs(eta)>2.1 && pt>300*GeV) pt=300*GeV;
  return m_diffDataMC[etaBin]->Eval(pt/GeV);
}
double JERProvider::getInsituDiffDataMCerror(double pt, double eta) {
  ...
  if (m_pt>1000*GeV) m_pt=1000*GeV; if (fabs(eta)>2.1 && m_pt>300*GeV) m_pt=300*GeV;
  return m_errorDataMC[etaBin]->Eval(m_pt/GeV)/100;
}
...
double JERProvider::getUncertainty(double pt, double eta, bool alt2, bool isAFII) {
  ...
  if ( isAFII && m_jetAlgo==AKt4LC && getEtaBin(eta)==0 && pt<25.0*GeV ) pt=25.0*GeV;
...
double JERProvider::getUncertainty_7TeV(double pt, double eta) {
  ...
  float factor = 0;
  if ( pt/GeV >= ptmin && pt/GeV <= ptmax) factor = 0;
  else if( pt/GeV > ptmax && pt/GeV <= ptpiv) factor = (pt/GeV - ptmax)/(ptpiv-ptmax);
  else if( pt/GeV > ptpiv) factor = 1;
...
double JERProvider::getDeltaInSitu(double pt, double y)
{
  ...
  mySyst.GetSRIsystematic(m_collectionName, pt/GeV, y, InSituSyst);
