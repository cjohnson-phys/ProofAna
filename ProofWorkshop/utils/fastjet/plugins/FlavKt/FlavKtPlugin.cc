...
void FlavKtPlugin::run_clustering(ClusterSequence & cs) const {
  ...
  // the the f90 code via its f77 interface
  ...
  for (int i = np; i > 0; i--) {
    ...
    if (parent1 > 0) {
      ...
      if (parent1 == -2) beam_flavour.backward  
                   = beam_flavour.backward - flavour_of(cs.jets()[lookup[parent2]]);
      else if (parent1 == -1) beam_flavour.forward  
                   = beam_flavour.forward  - flavour_of(cs.jets()[lookup[parent2]]);
      else {
