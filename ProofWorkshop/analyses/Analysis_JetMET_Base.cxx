...
bool Analysis_JetMET_Base::AddGrid(const MomKey ObjectKey){
...
void Analysis_JetMET_Base::AddGhostMatch(const MomKey JetKey, const MomKey MatchObject){
  ...
  if(JetTypeS.Contains("7"))
    ...
    baseR = 0.6;
  else if(JetTypeS.Contains("2"))
    baseR = 0.2;
  else if(JetTypeS.Contains("3"))
    baseR = 0.3;
...
MomKey Analysis_JetMET_Base::MakeJets(const fastjet::JetAlgorithm algo, const double jetR, const MomKey constType, const MomKey extra){
...
MomKey Analysis_JetMET_Base::MakeSKJets(const fastjet::JetAlgorithm algo, const double jetR, const MomKey constType, const double rapMax, const double grid_size, const bool doSoftKill){
  ...
  cout << "# Ran the following soft killer: " << killer.description() << endl;
...
//  cout << "Soft Killer applied a pt threshold of " << pt_threshold << endl;
  ...
  vector<fastjet::PseudoJet> ourJets;
  if(doSoftKill){
  	ourJets = kill_jets;
  } else{
  	ourJets = full_jets;
  ...
  for(unsigned int iJet = 0 ; iJet < ourJets.size() ; iJet++){
  	fastjet::PseudoJet jet = ourJets[iJet];
  	vector<fastjet::PseudoJet> constituents = jet.constituents();
  	Particle* jetP = new Particle();
  	jetP->p.SetPtEtaPhiE(jet.pt(), jet.eta(), jet.phi(), jet.e());
  	static const MomKey ConsKey("constituents");
