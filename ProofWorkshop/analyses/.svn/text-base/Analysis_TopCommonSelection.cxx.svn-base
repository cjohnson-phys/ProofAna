...
//-----------------------------------//
 ...
 for(int i=0; i<jets(TopCommonJets); i++){
   ...
   if(jet(i,TopCommonJets).Float("flavor_weight_MV1") > 0.7892){
     ...
     if (jet(i,TopCommonJets).Float("JVF") < 0.25 && jet(i,TopCommonJets).p.Perp() < 50.) continue;
 ...
 for(int iJ = 0; iJ < jets(TopCommonJets); iJ++){
   ...
   if(jet(iJ,TopCommonJets).Float("JVF") < 0.25 && jet(iJ,TopCommonJets).p.Perp() < 50.) continue;
