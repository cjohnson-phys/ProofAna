...
void ProofAna::Begin(TTree * /*tree*/)
{
  ...
  TFile *input = 0;
  TNamed *optionsfile = (TNamed*)fInput->FindObject("ProofAna_options");
  if(optionsfile)
    input = new TFile(optionsfile->GetTitle(),"READ");
  if(!input || input->IsZombie())
  	input = new TFile(fOptionsFile.Data(),"READ");
	if (!input->IsZombie()) {
   		AddDir(input,fInput);
   		input->Close();
   	}
	...
	while (lnk) {
		if(lnk->GetObject()->InheritsFrom(Config::Class())) {
			...
			if(TString(((TNamed*)lnk->GetObject())->GetName()).CompareTo("ProofAna")==0)  {
				...
				cout << " initialized eventbuilder! " << endl;
			}
			else {
...
Bool_t ProofAna::Process(Long64_t entry)
{
  ...
  Bool_t result = Process(entry, 1);

	fTimer->Stop(aProofAna_Process);
  
  return result;
}
Bool_t ProofAna::Process(Long64_t entry, Long64_t iter)
{
  ...
  if(result && iter < fCopies) result &= Process(entry,iter+1);
...
void ProofAna::SlaveTerminate()
{
	...
	fTimer->ShowStopped();
        cout << "SlaveTerminate ending" << endl;
}
...
Bool_t ProofAna::Notify()
{
	...
	cout << "ProofAna: DEBUG Starting to process file: " << EventBuilderBase::Filename() << endl;
...
void ProofAna::InitOutputFile(AnaKey name)
{
	...
	if(fFiles.find(name)==fFiles.end()) {
		...
		TString output(name.Data());
		if(name==adefault) output = fConfig->String(mDATASET);
		if(fConfig->Exists("SHORTOUTPUT") && fConfig->Bool("SHORTOUTPUT"))
			output = "output";
