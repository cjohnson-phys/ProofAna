...
namespace egRescaler {
  ...
  EnergyRescalerUpgrade::~EnergyRescalerUpgrade() {
	...
	if(m_zeeModel) delete m_zeeModel;
	if(m_zeeMethod) delete m_zeeMethod;
    if(m_resNom) delete m_resNom;
    if(m_psNom) delete m_psNom;
    if(m_psB12) delete m_psB12;
    if(m_r12Nom) delete m_r12Nom;
    if(m_r12B12) delete m_r12B12;
    
    if(m_matElectronScale) delete m_matElectronScale;
    if(m_matUnconvertedScale) delete m_matUnconvertedScale;
    if(m_matConvertedScale) delete m_matConvertedScale;
    if(m_eopEtaPosTimeDependence) delete m_eopEtaPosTimeDependence;
    if(m_eopEtaNegTimeDependence) delete m_eopEtaNegTimeDependence;
    if(m_G4OverAFII) delete m_G4OverAFII;
    if(m_G4OverFrSh) delete m_G4OverFrSh;
    
    if(m_psElectronEtaBins) delete m_psElectronEtaBins;
    if(m_psUnconvertedEtaBins) delete m_psUnconvertedEtaBins;
    if(m_psConvertedEtaBins) delete m_psConvertedEtaBins;
    if(m_matElectronEtaBins) delete m_matElectronEtaBins;
    if(m_lkgElectronEtaBins) delete m_lkgElectronEtaBins;
    
    if(m_psElectronGraphs) delete m_psElectronGraphs;
    if(m_psUnconvertedGraphs) delete m_psUnconvertedGraphs;
    if(m_psConvertedGraphs) delete m_psConvertedGraphs;
    if(m_matElectronGraphs) delete m_matElectronGraphs;
    if(m_lkgElectronGraphs) delete m_lkgElectronGraphs;
  ...
  void EnergyRescalerUpgrade::Init(std::string filename, std::string year, std::string esmodel) {
    ...
    if( year=="2012" ) {
      ...
      m_zeeNom =                     (TH1F*) m_erFile->Get("Scales/2012/ZScales2012_errStat_Full");
      ...
      m_zeeNom =                     (TH1F*) m_erFile->Get("Scales/2011/ZScales2011_errStat");
    ...
    if(m_zeeNom) m_zeeNom->SetDirectory(0);
	if(m_zeeModel) m_zeeModel->SetDirectory(0);
	if(m_zeeMethod) m_zeeMethod->SetDirectory(0);
    if(m_resNom) m_resNom->SetDirectory(0);
    if(m_psNom) m_psNom->SetDirectory(0);
    if(m_psB12) m_psB12->SetDirectory(0);
    if(m_r12Nom) m_r12Nom->SetDirectory(0);
    if(m_r12B12) m_r12B12->SetDirectory(0);
    
    //if(m_matElectronScale) m_matElectronScale->SetDirectory(0);
    //if(m_matUnconvertedScale) m_matUnconvertedScale->SetDirectory(0);
    //if(m_matConvertedScale) m_matConvertedScale->SetDirectory(0);
    //if(m_eopEtaPosTimeDependence) m_eopEtaPosTimeDependence->SetDirectory(0);
    //if(m_eopEtaNegTimeDependence) m_eopEtaNegTimeDependence->SetDirectory(0);
    if(m_G4OverAFII) m_G4OverAFII->SetDirectory(0);
    if(m_G4OverFrSh) m_G4OverFrSh->SetDirectory(0);
    
    if(m_psElectronEtaBins) m_psElectronEtaBins->SetDirectory(0);
    if(m_psUnconvertedEtaBins) m_psUnconvertedEtaBins->SetDirectory(0);
    if(m_psConvertedEtaBins) m_psConvertedEtaBins->SetDirectory(0);
    if(m_matElectronEtaBins) m_matElectronEtaBins->SetDirectory(0);
    if(m_lkgElectronEtaBins) m_lkgElectronEtaBins->SetDirectory(0);
    
    if(m_psElectronGraphs) m_psElectronGraphs->SetOwner(kTRUE);
    if(m_psUnconvertedGraphs) m_psUnconvertedGraphs->SetOwner(kTRUE);
    if(m_psConvertedGraphs) m_psConvertedGraphs->SetOwner(kTRUE);
    if(m_matElectronGraphs) m_matElectronGraphs->SetOwner(kTRUE);
    if(m_lkgElectronGraphs) m_lkgElectronGraphs->SetOwner(kTRUE);
