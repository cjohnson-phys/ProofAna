...
void runProof(TString url, TString dataset, int nwrks = -1, TString treename = "", bool forcetree = false, const char *optfile = 0)
{
    ...
    if(optfile)
      proof->SetParameter("ProofAna_options",optfile);
    proof->Process(dsname.Data(),sel.Data());
