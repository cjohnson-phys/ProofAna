...
int main() {
  ...
  for (size_t ji=0;ji<jetAlgos.size();++ji) {
    JERProvider myJER(jetAlgos[ji].Data());
    myJER.init();
    myJER.useGeV(true);
    can->SetLogx();
    for (size_t ieta=1;ieta<etaBins.size();++ieta) {
      ...
      DrawTextR("JER in 8 TeV data",0.85); DrawTextR("MC12 fullsim JER (truth)",0.8,kRed+1);
      DrawTextR("MC12 AFII JER (truth)",0.75,kBlue);
      can->Print(pdf);
    }
    cout << "hey" << endl;
    can->SetLogx(0);
    ...
    DrawJetAlgo(jetAlgos[ji],0.87); DrawTextL("#sqrt{#it{s}} = 8 TeV",0.82);
    can->Print(pdf); can->Print(pdfUnc);
...
void DrawJER(JERProvider &jer, double eta) {
  ...
  for (int i=10;i<2500;++i) {
    ...
    af2->SetPoint(point,pt,jer.getRelResolutionMC_AFII(pt,eta));
    data->SetPoint(point,pt,jer.getRelResolutionData(pt,eta));
    //data->SetPointError(point,0,jer.getInsituUncertData(pt,eta));
    data->SetPointError(point,0,jer.getInsituUncert(pt,eta));
    mc_up->SetPoint(point,pt,jer.getRelResolutionMC(pt,eta)+jer.getResolutionUncert(pt,eta));
    af2_up->SetPoint(point,pt,jer.getRelResolutionMC_AFII(pt,eta)+jer.getResolutionUncert_AFII(pt,eta));
  }
  pt_axis->Draw(); 
  data->SetFillStyle(3005); mc->SetFillStyle(3004); mc->SetFillColor(kRed+1);
  data->SetLineWidth(2); data->Draw("4"); data->Draw("lx");
  mc->SetLineColor(kRed+1); mc->SetLineWidth(2); mc->Draw("lx"); mc->Draw("4");
//  af2->SetLineColor(kBlue); af2->SetLineWidth(2); af2->Draw("l"); 
  mc_up->SetLineColor(kRed+1); mc_up->SetLineStyle(2); mc_up->Draw("l");
//  af2_up->SetLineColor(kBlue); af2_up->SetLineStyle(2); af2_up->Draw("l");
}
...
void DrawJERvsEta(JERProvider &jer, double pt) { 
  ...
  for (int i=-500;i<500;++i) {
    ...
    af2->SetPoint(point,eta,jer.getRelResolutionMC_AFII(pt,eta));
    ...
    af2_up->SetPoint(point,eta,jer.getRelResolutionMC_AFII(pt,eta)+jer.getResolutionUncert_AFII(pt,eta));
  }
  eta_axis->Draw();
