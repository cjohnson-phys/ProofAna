...
void runLocal(TString dataset, TString treename = "", Long64_t nentries=TChain::kBigNumber,  Long64_t firstentry = 0, int copies = 0)
{
	ProofAna* ana = new ProofAna(0,copies);
	...
	if(!mgr.ExistsDataSet(dataset)) {
		...
		//mgr.RegisterDataSet(dataset,dset); //This seems to return true regardless of success or failure at the moment
