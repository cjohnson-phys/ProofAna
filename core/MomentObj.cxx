...
struct abseta_cmp {
  ...
  bool operator ()(TObject* a, TObject* b) const {
    ...
    */
    ...
    if(TMath::Abs(TMath::Abs(x->p.Eta())-TMath::Abs(y->p.Eta()))<1e-5) return false;

    bool result = false;
    if (TMath::Abs(x->p.Eta()) > TMath::Abs(y->p.Eta())) result=true;
    else result=false;
...
void MomentObj::SortAbsEta(const MomKey& key, bool reverse) {
  static TClass* pcl = Particle::Class();
  vector<TObject*>* coll = Find(key)->second->ObjVec();

  if(coll->size()) {
    if((*coll->begin())->InheritsFrom(pcl)) {
      static abseta_cmp cmp;
      cmp.reverse = reverse;
      sort(coll->begin(),coll->end(),cmp);
    }
    else {
